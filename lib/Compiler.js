...
var Compiler = function () {
  ...
  _createClass(Compiler, [{
    ...
    key: 'compileFolder',
    value: function compileFolder(folderPath) {
      var _this2 = this;
        ...
        _fs2.default.readdir(folderPath, function (error, filePaths) {
          ...
          filePaths.forEach(function (filePath) {
            if (filePath.match(/\.(js|json)$/)) {
              schemas = [].concat(_toConsumableArray(_this2.extractSchemas(_path2.default.join(folderPath, filePath))), _toConsumableArray(schemas));
            }
          });
    ...
    value: function extractSchemas(filePath) {
      ...
      var pathsToResolve = [filePath];
      var schemas = [];

      // Use `require()` as it handles JSON and JS files easily

      var _loop = function _loop() {
        var resolvePath = pathsToResolve.shift();

        if (!resolvePath.match(/\.(js|json)$/)) {
          return 'continue';
        }

        var schema = new _Schema2.default(require(resolvePath));

        schema.path = resolvePath;
        schemas.unshift(schema);

        Object.keys(schema.references).forEach(function (ref) {
          pathsToResolve.push(_path2.default.normalize(_path2.default.join(basePath, schema.references[ref])));
        });
      };
    ...
    value: function generateOutput(schemas) {
      return new Promise(function (resolve) {
        ...
        schemas.forEach(function (schema) {
          ...
          var renderer = _Factory2.default.renderer(_config2.default.renderer, schema);
