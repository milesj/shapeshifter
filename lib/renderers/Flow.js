...
var FlowRenderer = function (_Renderer) {
  ...
  _createClass(FlowRenderer, [{
    ...
    value: function renderArray(definition, depth) {
      ...
      if ((0, _isPrimitive2.default)(configType) || configType === 'instance' || configType === 'array') {
        template += '[]';
      } else {
    ...
    value: function renderObject(definition, depth) {
      var key = this.renderAttribute(definition.keyType, depth + 1);
      var value = this.renderAttribute(definition.valueType, depth + 1);

      return this.wrapNullable(definition, this.formatObject(this.wrapProperty('[key: ' + key + ']', value, depth + 1), depth));
    }
